...
import type { EmployeeCollection, Employee, CompanyData } from '../../types';

interface EmployeesStepProps {
    employees: EmployeeCollection;
    onChange: (employees: EmployeeCollection) => void;
    companyData: CompanyData;
    onCompanyDataChange: (field: keyof CompanyData, value: string) => void;
    validationError?: string;
}

const EmployeesStep: React.FC<EmployeesStepProps> = ({
                                                         employees,
                                                         onChange,
                                                         companyData,
                                                         onCompanyDataChange,
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div style={{
                ...
                <h3 style={{fontSize: '18px', fontWeight: '600', color: 'var(--neutral-800)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px'}}>
                    <span>📊</span>
                    {TEXTS.LABELS.PLANNED_EMPLOYEE_COUNT}
                </h3>
                <p style={{fontSize: '14px', color: 'var(--neutral-600)', marginBottom: '16px'}}>
                    Określ dokładną liczbę pracowników, którzy będą objęci szkoleniem
                </p>

                <div style={{maxWidth: '200px'}}>
                    <Input
                        type="number"
                        value={companyData.planned_employee_count}
                        onChange={(e) => onCompanyDataChange('planned_employee_count', e.target.value)}
                        placeholder="np. 5"
                        min="1"
                        step="1"
                        style={{
                ...
                {plannedCount > 0 && (
                    <div style={{
                        ...
                        📋 Planowana liczba: <strong>{plannedCount}</strong> | Dodanych: <strong>{employeeCount}</strong>
                        {employeeCount === plannedCount && (
                            <span style={{color: 'var(--success-600)', marginLeft: '8px'}}>✅ Zgodne</span>
                        )}
                        {employeeCount < plannedCount && (
                            <span style={{color: 'var(--warning-600)', marginLeft: '8px'}}>⏳ Brakuje: {plannedCount - employeeCount}</span>
                        )}
                        {employeeCount > plannedCount && (
                            <span style={{color: 'var(--error-600)', marginLeft: '8px'}}>⚠️ Za dużo: +{employeeCount - plannedCount}</span>
                        )}
                    </div>
                )}
            </div>
